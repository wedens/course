...
instance Bind (State s) where
  (=<<) ::
    ...
    -> State s a
    -> State s b
  f =<< State k = State (\s -> let (a, t) = k s in runState (f a) t)
...
firstRepeat ::
  ...
  List a
  -> Optional a
firstRepeat l = eval (f l) S.empty
  where
    f (x :. xs) = get >>= (\s -> if S.member x s then return $ Full x else put (S.insert x s) *> f xs)
    f Nil = return Empty
